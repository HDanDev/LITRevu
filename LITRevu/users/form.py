...
class CustomUserEditForm(forms.ModelForm):
    ...
    def clean_profile_picture(self):
        ...
        if not (
            image.name.endswith(
                '.jpg'
                ) or image.name.endswith(
                    '.png'
                    )):
            errorFormat = "Only .jpg and .png files are allowed."
            raise forms.ValidationError(errorFormat)
    ...
    def clean_date_of_birth(self):
        ...
        if birth_date:
            ...
            min_birth_date = today - timezone.timedelta(
                days=12*365
                )  # 12 years ago
            max_birth_date = today - timezone.timedelta(
                days=200*365
                )  # 200 years ago
            ...
            if birth_date < max_birth_date:
                raise forms.ValidationError(
                    'You must be younger than 200 years old to register.'
                    )
...
class CustomAuthenticationForm(AuthenticationForm):
    def __init__(self, *args, **kwargs):
        ...
        self.fields['username'].widget.attrs.update(
            {'class': 'form-control', 'id': 'auth-username-field'}
            )
        self.fields['password'].widget.attrs.update(
            {'class': 'form-control', 'id': 'auth-password-field'}
            )
