    ...
    try:
        ...
        if default_token_generator.check_token(user, token):
            if not user.is_active:
                ...
                resent to your email address. <br />
                Did not receive it?
                <a href="/resend-activation-email/{uidb64}/{token}/">
...
def profile_view(request):
    ...
    is_following = Case(
        When(
            ...
            followers__status=True, then=True
            ),
        default=False,
    ...
    is_follower = Case(
        When(
            ...
            following__status=True, then=True
            ),
        default=False,
        output_field=BooleanField()
    )
    ...
    context.update({
        'col{}'.format(
            i
            ): colour_numbers[i] for i in range(
                min(
                    len(
                        colour_numbers
                        ), 10
...
def update_profile(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            old_profile_picture_path = pp.path if pp else None
            new_profile_picture = form.cleaned_data['profile_picture']
            if new_profile_picture and new_profile_picture != pp:
                try:
                    ...
                    oppp = old_profile_picture_path
                    if oppp and os.path.isfile(oppp):
                        try:
                            ...
                            print(e)
                            # msg = f'''An error occurred while
                            # deleting your previous profile picture: {e}'''
                            # return JsonResponse(
                            #     {
                            #         'success': False,
                            #         'errors': msg
                    ...
                    messages.error(
                        request,
                        f'An error occurred while updating your profile: {e}'
                        )
                    # return JsonResponse(
                    #     {
                    #         'success': False,
                ...
                user.save()
                msg = "Your date of birth has been successfully updated"
